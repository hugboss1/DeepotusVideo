...
* `windows(words, min_s=15, max_s=60, step_s=5)` — `words` = les mots
  ...
  `(start, end)`, plafonnées à `_MAX_WINDOWS`. Chacune :
...
* `score(wins, llm=None, n=4, persona=None)` →
  `{clips:[{i, start, end, score, title, hook, segments}], source}`.
  ...
  valide (i hors des fenêtres, score illisible) → HEURISTIQUE, dit par
  `source == "heuristique"` ; sinon `source == "llm:<fournisseur>"`. Au plus
  60 fenêtres partent au modèle (les 60 meilleures de l'heuristique, en
  ordre chronologique ; texte coupé à 280 caractères). Le choix du modèle
  est pris TEL QUEL (dédoublonné par `i`, trié par score, coupé à `n`) ;
  l'heuristique, elle, choisit gloutonnement des fenêtres qui ne se
  chevauchent pas, puis complète par les meilleures restantes si la source
  est trop courte pour `n` extraits disjoints. `n` borné 1..8.
...
def _clip(win: dict, sc, title="", hook="") -> dict:
...
def _heuristique(wins: list[dict], n: int, persona) -> list[dict]:
    notes = sorted(((heuristic(w, persona), w) for w in wins),
                   key=lambda p: (-p[0], p[1]["start"], p[1]["end"]))
    pris = []
    for sc, w in notes:
        if len(pris) >= n:
            break
        if all(w["end"] <= q["start"] or w["start"] >= q["end"] for _, q in pris):
            pris.append((sc, w))
    for sc, w in notes:                 # source trop courte : on complète
        if len(pris) >= n:
            break
        if all(w is not q for _, q in pris):
            pris.append((sc, w))
    pris.sort(key=lambda p: (-p[0], p[1]["start"]))
    return [_clip(w, sc) for sc, w in pris]
...
def _parse(texte: str, par_i: dict) -> list[tuple]:
    ...
    for it in data:
        ...
        try:
            ...
            continue
        if i not in par_i or i in vus or sc != sc:
            continue
        vus.add(i)
        out.append((max(0, min(100, int(round(sc)))), par_i[i],
...
def score(wins, llm=None, n=4, persona=None) -> dict:
    ...
    n = _bornes_n(n)
    if not wins:
        return {"clips": [], "source": "heuristique"}
    if llm is None:
        from app.services import summarizer
        llm = summarizer._chat_dispatch
    # les meilleures fenêtres de l'heuristique, en ordre chronologique
    cands = sorted(wins, key=lambda w: (-heuristic(w, persona), w["start"]))[:_LLM_WINDOWS]
    cands.sort(key=lambda w: w["start"])
    ...
    system = (f"Tu es le monteur de {str(persona or 'la chaîne')[:60]}. Tu choisis les "
              ...
              f'"hook": "<accroche>"}}], sans aucun texte autour.')
    ...
    return {"clips": [_clip(w, sc, t, h) for sc, w, t, h in choix[:n]],
            "source": "llm:%s" % (prov or "?")}
