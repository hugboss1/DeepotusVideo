...
lecture nue, une regex qui ne trouve rien fait ROUGIR le banc, pas mourir.
  ...
  [1] E-6 : chaque id de `DZM_MENU_RUB` (couche) EXISTE dans `SVM_ACTIONS`
      du bundle patche (45 entrees apres R_R1, 46 depuis L7 D-10) ; les ids de SVM_ACTIONS qui
...
NACT = 46
check("x1_le_bundle_patche_porte_quarante_six_actions_ids_uniques_quatre_sections",
...
check("x1_chaque_id_de_la_table_des_rubriques_existe_dans_SVM_ACTIONS",
      len(RUB) == 32 and len(IDS) == NACT and set(RUB) <= set(IDS), sorted(set(RUB) - set(IDS)))
...
check("x1_les_quatorze_ids_hors_table_sont_les_gestes_de_tete_Lecture_ou_Montage_vers_Timeline",
      len(RUB) == 32 and len(IDS) == NACT and HORS == TETE and all(SEC[i] in ("Lecture", "Montage") for i in HORS),
      sorted(HORS ^ TETE))
...
check("x2_aller_retour_svmComboOfEvent_rend_la_combo_de_depart_pour_les_46",
