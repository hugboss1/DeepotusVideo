...
check("x1_la_table_porte_trente_quatre_ids_et_six_rubriques_moins_Projet_et_Timeline",
      # L6 D-26 (25/09/2026, tache 6) : 36 -> 37 ids -- vo_record range en Édition (sec « Audio » : le repli dirait la meme
      # rubrique, la table le DIT) ; libelle garde, valeur realignee
      len(RUB) == 37 and RUB.get("vo_record") == "Édition"
      and RUB.get("grade_copy") == "Édition" and RUB.get("grade_paste") == "Édition" and set(RUB.values()) == {"Édition", "Marqueurs", "Affichage", "Aide"}, (len(RUB), sorted(set(RUB.values()))))
...
# 50 -> 51 le 25/09/2026 (L6 D-26, tache 6) : `vo_record` (« Alt+R », sec Audio), repli dans R_R1 ; rangee en Édition
# par DZM_MENU_RUB (36 -> 37 ids) ; l'aller-retour [2] la joue aussi (Alt -> lettre PHYSIQUE KeyR -> « Alt+R »).
NACT = 51
...
check("x1_chaque_id_de_la_table_des_rubriques_existe_dans_SVM_ACTIONS",
      len(RUB) == 37 and len(IDS) == NACT and set(RUB) <= set(IDS), sorted(set(RUB) - set(IDS)))
...
check("x1_les_quatorze_ids_hors_table_sont_les_gestes_de_tete_Lecture_ou_Montage_vers_Timeline",
      len(RUB) == 37 and len(IDS) == NACT and HORS == TETE and all(SEC[i] in ("Lecture", "Montage") for i in HORS),
      sorted(HORS ^ TETE))
...
check("x1_les_quatre_marqueurs_vont_en_Marqueurs_keys_panel_en_Aide_undo_en_Edition",
      len(RUB) == 37 and all(RUB.get(k) == "Marqueurs" for k in ("marker_toggle", "marker_prev", "marker_next", "marker_index"))
