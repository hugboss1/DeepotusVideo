...
def test_la_geometrie_vit_dans_forge3d_scene_et_le_stl_est_deux_passes():
    ...
    forge3d réexporte (compat) ; le writer STL ne matérialise plus la géométrie
    en tuples (mesure : l'ancien écrivait ~160 Mo d'intermédiaires par relief
    au grid max) — on prouve l'ÉQUIVALENCE des octets et la structure."""
    import importlib
