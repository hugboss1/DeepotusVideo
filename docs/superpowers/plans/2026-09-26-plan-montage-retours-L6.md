...
- [x] T6 — clôture (mutations, banc croisé, preuve écran, conception/mémoire datées, revue finale, PR) (`15d1ba7` restes de revue, `8763250` 25 mutations rouges + `test_retours_croise.py` 21/0 (table partagée `retours_bornes_vecteurs.json` 66 cas, `dzmGlActif` == `_au_temps`), `fb3ead4` conception datée ; revue finale PRÊT À FUSIONNER (30 commandes + 6 appels grade/scopes identiques à `62f0c75` hors écho/réverbe et musique bornée) ; preuve écran 8799 VERTE le 26/09 (`preuve_retours.mjs`) : image étalonnée 170×302 au format du cadre à l'arrêt, masquée en lecture, écart 5,5/255 à l'image du rendu Preview au même instant (témoin source brute 247,7) ; scopes flottants déplacés et redimensionnés 320→520 sans requête pendant le geste, une au relâcher (`size`, `cadre` avec `dur`), géométrie gardée après F5 ; musique 440 Hz présente 0–3 s (clip) puis −inf jusqu'à 8 s ; écarts datés de la revue finale : `_cadre_pre` passe `fps=30` en dur à `_dz_filter` (sans effet sur une image unique), `reframe` normalisé avec srcIn 0 (plafond de 240 points sur toute la source, écart possible au-delà de 240 points de suivi))
