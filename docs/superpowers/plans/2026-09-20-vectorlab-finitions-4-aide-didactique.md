...
- **Livré** : `mod-didact.js` (pur : `fiche_pour`, `compter_mots`, `valider_fiche`, `didact_html`, `DIDACT_DELAI_MS = 900`, `DIDACT_LARGEUR = 320` ; DOM : survol long en capture sur `document`, encart `#vlDidact.vl-didact` positionné par `bulle_position`, bouton « ? » `.vl-didact-q` posé après chaque option à fiche via `MutationObserver`, Échap / clic ailleurs ferment, `VL.didact`) + CSS aux jetons + `aide/index.json` — `8e3be59` ; **six fiches** WebP 320 × 200 × 3 temps × 1,2 s en boucle : `pixelui.pxContour` (`fb9c82e`), `impression.impDepouille` + `pixelui.pxCreerCalque` (`9003a19`), `pixelui.pxDesigner` + `pixelui.pxPixeliser` + `apparence2.a2Conique` (`799a21c`) ; **skill `C:\Users\olivi\.claude\skills\didacticiel-option\`** : `SKILL.md` (déroulé en six temps, interdits), `scripts/capture.js` (sérialisation `#canvasHost svg` + `#overlay` → canvas 640 × 400 → PNG ; halo, curseur, chip ; `projection` orthographique d'un maillage réel), `scripts/assembler.py` (Pillow, WebP animé, APNG en repli, refuse des temps identiques), `scripts/verifier.py` (index, ≤ 25 mots, 320 × 200, 3 frames distinctes, boucle, ≥ 1000 ms), `references/procedure.md`, `evals/evals.json` (Contour sombre, Dépouille, Créer le calque pixel, avec assertions).
- **TDD** : `qa/didact.test.mjs` RED (module absent) → GREEN 8 contrôles ; `run.mjs` intégral vert. Les trois évals du skill ont été EXÉCUTÉES inline (fiches produites par la procédure elle-même) ; pas de viewer ni de baseline « sans skill » (session autonome, aucune validation en cours de route).
- **Prouvé** (8799, 1400 × 900, `onerror` capturé, zéro erreur) : chaque fiche = trois captures distinctes prises DANS la page après un déclenchement RÉEL de l'option — Contour sombre : clic `#pxContour`, `rev` 0 → 1, 68 pixels sombres apparus ; Dépouille : maillage `extruder_depouille` 12 tris (0°) → 264 tris (+20°), curseur `#impDepouilleR` posé à 20 ; Créer le calque pixel : modèle désigné (`pixelart.modele = o1`, cellule 16) puis calque « pixel » avec `o2` créé, sélection `o2` ; Désigner comme modèle : `pixelart.modele` posé ; Pixeliser : `nat` 32 × 32 → 8 × 8 (rendu `pixelated` conservé à la capture) ; Conique : `style.fond = grad:g1`. Survol long mesuré sur les six ids : rien à 500 ms, encart visible à 1 100 ms, 320 × 279 px, `img.naturalWidth = 320` (webp `?v=1`), titre et phrase (13 à 21 mots), bouton « ? » présent et fonctionnel (clic → ouvre ; Échap → ferme). `verifier.py` = OK ×6.
- **Déployé** : 11 fichiers vers `%LOCALAPPDATA%\DeepotusVideoGen` (installé = base `7d3d6a1` pour `core.js` / `vectorlab.css`, 9 neufs), sauvegarde `_backup_predeploy_2026-09-20-didactique`, installé = cible par `git hash-object` ; `GET :8765/vectorlab/aide/index.json` 200, `pixelui.pxContour.webp` 200 (42 276 o). Statiques seuls : aucune relance.
- **Reste / écarts** : le panneau HTML n'est pas sérialisé (le geste est montré par un halo sur l'objet + un chip nommant l'option) ; la Dépouille ne montre pas le `<model-viewer>` (vide en volet caché) mais une vue de côté projetée du maillage réel ; la fiche « Pixel-art » des captures est « Pixeliser l'image » (`#pxPixeliser`, effet visible) plutôt que « Tuile W × H » (effet invisible sur la scène) ; le skill ne porte pas d'optimisation de description (`run_loop.py`) ni de benchmark viewer.
