...
/* ── HSL (handoff Vectorlab §8.2) : l'éditeur de teinte du Panneau de
   verre parle en Teinte / Saturation / CLARTÉ — l'algorithme standard,
   arrondi entier sur les trois canaux, aucune dépendance. */
export function rgbVersHsl({ r, g, b }) {
  const rn = r / 255, gn = g / 255, bn = b / 255;
  const max = Math.max(rn, gn, bn), min = Math.min(rn, gn, bn);
