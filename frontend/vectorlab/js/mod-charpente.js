...
export function initCharpente(VL) {
  ...
  const T = {
    ...
    calqueRenommer: { peut: () => !!calqueActif(), faire: () => { const c = calqueActif(); const nom = prompt("Nom du calque :", c.nom || ""); if (nom !== null) VL.executer(op_calque_renommer, c.id, nom); } },
    calqueSupprimer: { peut: () => !!(etat.doc && etat.doc.calques.length > 1 && calqueActif()), faire: () => { const c = calqueActif(); if (confirm(`Supprimer le calque « ${c.nom} » et ses objets ?`)) { VL.executer(op_calque_supprimer, c.id); if (!calqueActif()) { etat.calqueActif = etat.doc.calques[etat.doc.calques.length - 1].id; VL.rendre(); } } } },
